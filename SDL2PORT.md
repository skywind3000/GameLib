...
下面这条命令适合测试图片、字体、音频、Tilemap、裁剪等完整路径，例如编译 `tests/sdldemo15.cpp`：

```bash
g++ -O2 -Wall -std=c++11 -fstrict-aliasing tests/sdldemo15.cpp -o sdldemo15.exe \
...
- 当前 SDL 回归入口是 `tests/sdldemo1.cpp` ~ `tests/sdldemo15.cpp`。
- 最新裁剪综合示例是 `tests/sdldemo15.cpp`，对应 Win32 的 `examples/19_clip_tilemap.cpp`。
- `examples/18_tilemap_file.cpp` 这种 Tilemap 文件保存/回读流程目前还没有单独的 SDL demo，对应功能虽已实现，但还缺一份独立示例。